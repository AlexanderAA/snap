...
module Snap.Snaplet.Auth.HandlersIO where
...
------------------------------------------------------------------------------
                    ...
                    ------------------------------------------
                    -- IO version of Higher level functions --
                    ------------------------------------------

------------------------------------------------------------------------------
-- | Create a new user from just a username and password inside of IO monad.
--
createUserIO :: IAuthBackend r =>
                 r                 -- ^ An auth backend
              -> Text              -- ^ Username
              -> ByteString        -- ^ Password
              -> IO (Either AuthFailure AuthUser)
createUserIO r unm pwd
  | null $ strip unm = return $ Left UsernameMissing
  | otherwise = do
     uExists <- usernameExistsIO r unm
     if uExists then return $ Left DuplicateLogin
                else buildAuthUser r unm pwd


------------------------------------------------------------------------------
-- | Check whether a user with the given username exists.
--
usernameExistsIO :: IAuthBackend r => r
